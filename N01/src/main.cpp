...
#include "solvers/thomas_solver.hpp"
...
auto main(int argc, char **argv) -> int {
  if (argc == 1) {
    std::cout << (solve_mat_thomas(10)).head(5) << "\nVEC2:\n";
    // std::cout << (solve_d_mat_fullpiv_qr(200)).head(5) << "\n";
...
BenchData parse_b_flag(int argc, char **argv) {
  ...
  } else if (method == "THOMAS_ALG") {
    bd.fn = [](long N) { solve_mat_thomas(N); };
  } else {
...
void gen_p_flag_data(char **argv) {
  ...
  Eigen::VectorXd u = solve_mat_thomas(N);
  for (int i = 0; i < N; i++) {
    // save data to file
    float64_t x = i * h;
    float64_t y = u(i);
